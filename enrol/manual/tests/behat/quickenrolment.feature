...
Feature: Teacher can search and enrol users one by one into the course
  ...
  @javascript
  Scenario: Create enrolment add fill endtime for form create enrol.
    Given I navigate to course participants
    ...
    Then I set the field "Not enrolled users" to "Student 002 (student002@example.com)"
    And I set the field "Starting from" to "2"
    And I set the field "timeend[enabled]" to "1"
    And I set the field "timeend[day]" to "10"
    When I press "add"
    Then the "Enrolled users" select box should contain "Student 002 (student002@example.com)"
    When I am on "Course 001" course homepage
    And I navigate to course participants
    Then I should see "Student 002"
    When I click on "Edit enrolment" "icon" in the "Student 002" "table_row"
    And the field "timeend[day]" matches value "10"

    # Update Enrol user to course
    When I am on "Course 001" course homepage
    Then I navigate to course participants
    And I click on "Edit enrolment" "icon" in the "Student 099" "table_row"
    Then the field "timeend[day]" matches value "10"
    And the "Enrolment duration" "select" should be enabled
    # fill duration
